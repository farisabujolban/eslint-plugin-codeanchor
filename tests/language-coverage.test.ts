...
tsTester.run('todo-requires-issue [TypeScript]', todoRequiresIssue, {
  valid: [
    { code: 'const x: string = "hello" // TODO: GH-10 update later' },
  ],
  invalid: [
    { code: '// TODO: refactor types', errors: [{ messageId: 'missingIssue' }] },
  ],
})

jsxTester.run('todo-requires-issue [JSX]', todoRequiresIssue, {
  valid: [
    { code: 'const el = <div /> // TODO: #99 fix layout' },
  ],
  invalid: [
    { code: 'const el = <div /> // TODO: no ref', errors: [{ messageId: 'missingIssue' }] },
  ],
})
...
tsTester.run('temp-comment-requires-condition [TypeScript]', tempCommentRequiresCondition, {
  valid: [
    { code: 'type Foo = string // TEMP: remove when #88 lands' },
  ],
  invalid: [
    { code: '// WIP: halfway done', errors: [{ messageId: 'missingCondition' }] },
  ],
})

jsxTester.run('temp-comment-requires-condition [JSX]', tempCommentRequiresCondition, {
  valid: [
    { code: 'const el = <span /> // TEMP: remove when #12 merged' },
  ],
  invalid: [
    { code: '// REMOVE: leftover code', errors: [{ messageId: 'missingCondition' }] },
  ],
})
...
plainJsTester.run('comment-expiry-date [plain JS]', commentExpiryDate, {
  valid: [
    { code: '// TODO: no date here, fine' },
    { code: '// FIXME: revisit this later' },
  ],
  invalid: [
    {
...
jsxTester.run('comment-expiry-date [JSX]', commentExpiryDate, {
  valid: [
    { code: 'const el = <div /> // NOTE: no date, not flagged' },
  ],
  invalid: [
    {
...
tsTester.run('no-placeholder-values [TypeScript]', noPlaceholderValues, {
  valid: [
    { code: 'const real: string = "actual-value"', filename: 'main.ts' },
  ],
  invalid: [
    {
...
jsxTester.run('no-placeholder-values [JSX]', noPlaceholderValues, {
  valid: [
    { code: 'const el = <Button label="Submit" />', filename: 'main.jsx' },
  ],
  invalid: [
    {
...
tsTester.run('no-hardcoded-connection-string [TypeScript]', noHardcodedConnectionString, {
  valid: [
    { code: 'const url: string = process.env.DB_URL!', filename: 'main.ts' },
  ],
  invalid: [
    {
...
jsxTester.run('no-hardcoded-connection-string [JSX]', noHardcodedConnectionString, {
  valid: [
    { code: 'const el = <DB url={process.env.DB_URL} />', filename: 'main.jsx' },
  ],
  invalid: [
    {
