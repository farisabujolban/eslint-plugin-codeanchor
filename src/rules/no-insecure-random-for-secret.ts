...
function containsMathRandom(node: unknown): boolean {
  ...
  for (const val of Object.values(n)) {
    ...
    if (Array.isArray(val) && val.some((v: unknown) => v && typeof v === 'object' && (v as Record<string, unknown>).type && containsMathRandom(v))) return true
  }
  return false
}
  ...
  create(context) {
    return {
      ...
      Property(node) {
        ...
        const key = node.key
        const keyName =
          key.type === 'Identifier' ? (key as { name: string }).name
          : key.type === 'Literal' ? String((key as { value: unknown }).value)
          : null
