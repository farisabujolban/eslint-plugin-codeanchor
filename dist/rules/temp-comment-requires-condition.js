...
export const tempCommentRequiresCondition = {
    meta: {
        type: 'suggestion',
        docs: {
            description: 'Require temporary/workaround comments to include an issue reference or removal condition',
            recommended: true,
        },
        schema: [
            {
        ...
        messages: {
            missingCondition: '{{keyword}} comment must specify when it should be removed (issue reference or removal condition).',
            missingIssue: '{{keyword}} comment must include an issue reference (#123, GH-123, PROJ-123, or a URL).',
        },
    },
    create(context) {
        ...
        function hasRemovalCondition(text) {
            if (hasIssueReference(text))
                return true;
            if (!requireIssue && CONDITION_RE.test(text))
                return true;
            return false;
        }
        return {
